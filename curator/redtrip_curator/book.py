...
from .personas import CURATOR_BLANK_HEADING, HEIDAI_NAME
...
def _review_html(data: dict[str, Any]) -> str:
    parts = [
        f'<section class="review"><h2>{html.escape(CURATOR_BLANK_HEADING)}</h2>'
        '<span class="ornament-line"></span>'
    ...
    if data["concerns"]:
        ...
        parts.append(
            f'<div class="review-concerns"><span class="k">{html.escape(HEIDAI_NAME)} · 反对意见</span><ul>'
            + "".join(items)
            + "</ul></div>"
        )
    extras = []
...
def _review_markdown(data: dict[str, Any]) -> list[str]:
    ...
    L: list[str] = ["", f"## {CURATOR_BLANK_HEADING}", "",
...
def _review_epub_inner(data: dict[str, Any]) -> str:
    parts = [
        f"<h2>{html.escape(CURATOR_BLANK_HEADING)}</h2>"
        '<p class="review-note">以下为对抗性评审留下的未决问题，不构成定论；'
        "它们指向值得在下一版或现场继续追问的方向。</p>"
    ]
    if data["warnings"]:
    ...
    if data["concerns"]:
        ...
        parts.append(f'<p class="k">{html.escape(HEIDAI_NAME)} · 反对意见</p><ul>' + "".join(items) + "</ul>")
    extras = []
    if data["missed_voices"]:
